...
class PytorchMultiHeadSelfAttHead(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        x_att_output = attn_output.reshape(input_shape)
        ...
        x = self.layer_norm(x + x_att_output)

        return x, attn_weights
    
class AdditiveWordAttention(nn.Module):
    def __init__(self, embedding_dimension, additive_vector_dim=300):
        super().__init__()
...
with open('final/train_acc.npy', 'wb') as f:
    np.save(f,np.array(train_accuracies))

with open('final/train_loss.npy', 'wb') as f:
